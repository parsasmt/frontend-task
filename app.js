...
// Show edit chart
window.showEditChart = function (icon) {
    const row = icon.closest('tr');
    const rowId = row.id;

    // Prepare chart data
    const labels = ['Edits'];
    const data = [editCounts[rowId] || 0];
    const ctx = document.getElementById('edit-chart').getContext('2d');

    if (window.editChart) {
        window.editChart.destroy(); // Destroy previous chart instance if exists
    }

    window.editChart = new Chart(ctx, {
        type: 'bar',
        data: {
            labels: labels,
            datasets: [{
